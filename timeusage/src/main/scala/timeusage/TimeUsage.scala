...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val headColumn: Array[StructField] = Array(StructField(columnNames.head, StringType, false))
    val otherColumns: Array[StructField] = columnNames.tail.toArray.map(columnName => StructField(columnName, DoubleType, false))
    StructType(headColumn ++ otherColumns)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsSet = Set("t01", "t03", "t11", "t1801", "t1803")
    val workingSet = Set("t05", "t1805")
    val otherSet = Set("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")
    val primaryWorkingSet = primaryNeedsSet ++ workingSet

    (
      columnNames.filter(name => primaryNeedsSet.contains(name.take(3)) || primaryNeedsSet.contains(name.take(5)))
        .map(name => col(name)),
      columnNames.filter(name => workingSet.contains(name.take(3)) || workingSet.contains(name.take(5)))
        .map(name => col(name)),
      columnNames.filter(name =>
        (!primaryWorkingSet.contains(name.take(3)) && !primaryWorkingSet.contains(name.take(5))) &&
          (otherSet.contains(name.take(3)) || otherSet.contains(name.take(5))))
        .map(name => col(name))
    )
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = {
      when(df("telfs").between(1, 2), "working").otherwise("female").as("sex")
    }
