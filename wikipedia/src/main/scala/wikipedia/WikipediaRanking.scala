...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local[8]").setAppName("wikipedia")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.filter(wiki => wiki.mentionsLanguage(lang))
      .aggregate(0)((acc, _) => acc + 1, (x, y) => x + y)
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.mapValues(f => f.size).collect().toList.sortWith(_._2 > _._2)
  }
